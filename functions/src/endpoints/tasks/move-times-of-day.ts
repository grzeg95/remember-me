...
export const handler = async (request: CallableRequest) => {
  ...
  return firestore.runTransaction(async (transaction) => {
    ...
    const roundIndex = round.timesOfDay.indexOf(data.timeOfDay);

    testRequirement(roundIndex === -1);
    testRequirement(data.move > 0 && roundIndex + data.move >= round.timesOfDay.length);
    ...
    arrayMove(newTimesOfDay, roundIndex, roundIndex + data.move);
    arrayMove(newTimesOfDayCardinality, roundIndex, roundIndex + data.move);
