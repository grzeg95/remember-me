import {DocumentData, DocumentReference, QueryDocumentSnapshot, DocumentSnapshot, FirestoreDataConverter} from 'firebase-admin/firestore';
...
export class Today implements TodayDoc {
  ...
  static ref(roundRef: DocumentReference<Round, RoundDoc>, id: string) {
    return roundRef.firestore.doc([roundRef.path, Collections.today, id].join('/')).withConverter(Today._converter);
  }

  static refs(roundRef: DocumentReference<Round, RoundDoc>) {
    return roundRef.firestore.collection([roundRef.path, Collections.today].join('/')).withConverter(Today._converter);
  }
